...
func _init(nlibrary:Dictionary, nsize:int=20, defaultTileValue:int=-1):
	size = nsize
	library = nlibrary
...
func tileMatch(library):
	...
	while i < size*size:
		...
		for stampName in library:
			...
			unlock = pattern.size()
			stampSize = int(stamp["size"])
			checkPos = Vect.new()
			
			#loop through stamp pattern, reuse unlock because it is size of pattern
			for j in range(unlock):
				checkPos.x = pos.x + j%stampSize
				checkPos.y = pos.y + j/stampSize
				if checkPos.x < size and checkPos.y < size:
					if getTile(checkPos.x,checkPos.y) == pattern[j]: #compare tiles with pattern to match
						...
						break
			if unlock <= 0:
				placeStamp(pos.x, pos.y, stamp["stamp"], stampSize)
				coverCheck -= 2
				matchFound = true
#				yield()
		i += 1
		if i >= size*size:
			if matchFound or coverCheck > size*size*0.2:
				print(str(coverCheck) + ":" + str(size*size*0.2))
				matchFound = false
				i = 0
			else:
				break
...
func placeStamp(x, y, stamp, stampSize):
	...
	for i in range(stamp.size()):
		setTile(x + i%stampSize, y + i/stampSize, stamp[i])
...
func setTile(x:int, y:int, tile:int):
	layout[ y * size + x] = tile

func getTile(x:int, y:int):
	return layout[ y * size + x]

func indexToXY(i):
	return Vect.new(i%size,i/size)
